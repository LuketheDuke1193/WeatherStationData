...
#include <MySQL_Connection.h> //MySQL connector library
#include <MySQL_Cursor.h> //MySQL Cursor library to use with MySQL connector libary.
#include <NTPClient.h> //This and WiFiUdp to be used for connection to MySQL database via hostname instead of IP address since it changes with AWS RDS
#include <WiFiUdp.h>
...
#define RxPin D5    //RX module connected to D5 on NodeMCU board.
#define ledPin LED_BUILTIN  //Human feedback CHANGE Changed to builtin.
...
void setup() {
  ...
  Serial.println(WiFi.localIP());         // Send the IP address of the ESP8266 NodeMCU to the computer
  ...
  if (conn.connect(server_ip, 3306, user, dbpassword)) {
    yield();
    Serial.println("Connection to database successful!");
...
void loop(){
  timeClient.update(); //Updates time client to current time.
...
void analyseData(){
  ...
  if (conn.connected() && minutesValid() && humidityTempNotZero()&& statementSent == false) {
    insertStatement.concat("INSERT INTO WeatherData.Readings (averageSpeed, gusts, direction, totalRain, rainRate, temperature, humidity) VALUES (");
        insertStatement.concat(avWindspeed);
        insertStatement.concat(", ");
        insertStatement.concat(gustWindspeed);
        insertStatement.concat(", ");
        insertStatement.concat("'");
        insertStatement.concat(windDir[quadrant]);
        insertStatement.concat("'");
        insertStatement.concat(", ");
        insertStatement.concat(rainTotal);
        insertStatement.concat(", ");
        insertStatement.concat(rainRate);
        insertStatement.concat(", ");
        insertStatement.concat(temperature);
        insertStatement.concat(", ");
        insertStatement.concat(humidity);
        insertStatement.concat(");");
        const int str_len = insertStatement.length() + 1;
        char insertChar[str_len];
        insertStatement.toCharArray(insertChar, str_len);
        Serial.println(insertStatement);
        cursor->execute(insertChar);
        statementSent = true;
        delay(1000);
      }
      if (!minutesValid() && statementSent == true){
        statementSent = false;
      }
}
